...
def getDictWords(text, word_type):
	...
	try:
		...
		class_link = class_link.replace("\n", " ").replace("\t", "").replace("\r", "").replace("\\r", "").replace("&nbsp;", " ").replace("\'", "'").replace("\\'", "'").replace("&quot;", "\"").replace("\\;", "").replace("\'", "'").replace("&lt;", "<").replace("&gt;", ">").replace("&trade;", "(TM)").replace("&#8212;", "--").replace("&#8220;", "\"").replace("&#8221;", "\"").replace("\\n", " ").replace("\\t", "").replace("&#8217;", "'")
		...
		if word_type == "definition":
			...
			description = str(description[:700]) + "\n\nScrabble Point: %s\nWord With Friends Point: %s" % (scrabble, word_with_friend)
